        ...
        <v-container grid-list-xl text-xs-center>
          <v-layout row wrap>
            <v-flex>
              ...
              <v-btn @click="check_itemlist()">조회하기</v-btn>
            </v-flex>
          </v-layout>
        </v-container>
          ...
          <v-layout row wrap>
            <v-flex v-for="i in this.uniq" :key="i" xs4 style="padding-bottom:80px;">
              <a style="color:black">
                <v-img
                  v-ripple
                  @click="pass_id(all_info[i].object_id)"
                  :src="`https://s3.ap-northeast-2.amazonaws.com/wearever1/`+all_info[i].imageUrl[0]"
                  class="image"
                  alt="lorem"
                  contain
                  aspect-ratio="1.1"
    ...
    check_itemlist() {
      ...
      for (var i = 0; i < this.all_info.length; i++) {
        ...
        if (store.state.itemAvailable !== "") {
          if (
            (store.state.itemAvailable !== "대여 중" &&
              this.all_info[i].status === 1) ||
            (store.state.itemAvailable !== "대여 가능" &&
              this.all_info[i].status === 0)
          ) {
            use_cnt--;
          }
        }
      ...
      // 정렬 및 중복 제거
      this.uniq = picked_cnt
        .slice() // 정렬하기 전에 복사본을 만든다.
        .sort(function(a, b) {
          return a - b;
        })
        .reduce(function(a, b) {
